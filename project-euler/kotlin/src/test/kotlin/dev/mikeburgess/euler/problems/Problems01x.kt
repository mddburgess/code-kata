...
class Problems01x {
    ...
    fun `Problem 11`() {
        ...
        val result = listOf(
            grid.mapIndexed { row, col, value ->
                value * grid[row, col + 1] * grid[row, col + 2] * grid[row, col + 3]
            },
            grid.mapIndexed { row, col, value ->
                value * grid[row + 1, col] * grid[row + 2, col] * grid[row * 3, col]
            },
            grid.mapIndexed { row, col, value ->
                value * grid[row + 1, col + 1] * grid[row + 2, col + 2] * grid[row + 3, col + 3]
            },
            grid.mapIndexed { row, col, value ->
                value * grid[row + 1, col - 1] * grid[row + 2, col - 2] * grid[row + 3, col - 3]
            }
        ...
        assertThat(result).isEqualTo(70600674L)
    }
}
